...
asio::awaitable<expected<void, WSError>> run()
{
    ...
    for (int i = 0;; i++)
    {
        ...
        {
            // // this would cause an ASIO lifetime issue because
            // // the handler is not guaranteed to be executed before
            // // the close call finishes, which would then use
            // // a destroyed WebSocketClient instance.
            // asio::co_spawn(
            //     write_strand,
            //     [&]() -> awaitable<void>
            //     {
            ...
            logger.log<LogLevel::E, LogTopic::User>(
                std::format("Closing with close code {}", to_string(err->close_with_code))
            );
