// Original source code based on: https://github.com/vog/sha1/
// Licensed as 100% Public Domain.

// Modifications for this library on 2024/01/20:
//    * Wrapped in namespace ws_client to avoid name collision with OpenSSL
//    * Added static_cast to avoid compiler warnings due to implicit conversions
//    * Added final_bytes() function to return the hash as a vector of bytes
//    * Removed from_file() function
//    * Removed final() function
...
inline std::vector<unsigned char> SHA1::final_bytes()
{
    /* Total number of hashed bits */
    uint64_t total_bits = (transforms * BLOCK_BYTES + buffer.size()) * 8;

    /* Padding */
    buffer.append(1, (char)0x80);
    size_t orig_size = buffer.size();
    buffer.append(BLOCK_BYTES - orig_size, (char)0x00);
    ...
    result.reserve(sizeof(digest) / sizeof(digest[0]) * 4);
    for (size_t i = 0; i < sizeof(digest) / sizeof(digest[0]); i++)
    {
        result.push_back(static_cast<unsigned char>((digest[i] >> 24) & 0xFF));
        result.push_back(static_cast<unsigned char>((digest[i] >> 16) & 0xFF));
        result.push_back(static_cast<unsigned char>((digest[i] >> 8) & 0xFF));
        result.push_back(static_cast<unsigned char>(digest[i] & 0xFF));
    }
